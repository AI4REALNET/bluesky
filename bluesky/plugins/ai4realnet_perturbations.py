...
class PerturbationGenerator(core.Entity):  
    def __init__(self):
        ...
        # print(f'initialised perturbations')
    ...
    def add_perturbations(self, perturbation_type: str, flag: str = 'ON'):
        ...
        # print(f'Adding perturbation: {perturbation_type}, type {type(perturbation_type)} set to {flag} type {type(flag)}')
    ...
    def update_disturbance(self):
        """
        Implements weather perturbations as drifting storm cells and volcanic as static ash clouds.

        Spawning:
            - With probability pweather/pvolcanic (each call), spawn a new disturbance if none is active.
            - Lifetime is sampled from an exponential distribution with mean qweather/qvolcanic (Poisson).
            

        Evolution:
            - Radius follows a triangular profile: base -> peak -> base over lifetime.
            - Weather: center drifts with constant (sampled) heading/speed.
            ...
            if self.weather_active:
                ...
                if (simt - self.weather_disturbance_start) >= self.weather_cell_lifetime:
                    ...
                    if self.weather_shape_jitter is None:
                        ...
                        print(f'Is this ever triggered?')
                    else:
                        jitter = 0.05
                        self.weather_shape_jitter = 1.0 + np.random.uniform(-jitter, jitter, size=self.n_verts)
        ...
        if self.volcanic_perturbation_flag:
            ...
            if self.volcanic_active:
                ...
                if (simt - self.volcanic_disturbance_start) >= self.volcanic_cell_lifetime:
                    # Delete volcanic disturbance (expired)
                    stack.process(f"DELETE {shape_name}")
                    self.volcanic_active = False
                    ...
                    # stack.process(f"CIRCLE {shape_name}_bounding_circle, {self.volcanic_cell_center_lat}, {self.volcanic_cell_center_lon}, {self.volcanic_cell_radius/constants_general.NM2KM}")
                    # stack.process(f"COLOR {shape_name}_bounding_circle, YELLOW")

            else: # No active cell: sample a new one with probability pvolcanic

                # If there is no volcanic distrubance active, sample with probality pvolcanic
                ...
                if volcanic_disturbance_flag:
                    ...
                    # Sample lifetime ~ Exponential(mean=qvolcanic)
                    # (Poisson process waiting time)
                    ...
                    # Jitter the vertices of each volcanic cell
                    jitter = 0.25
                    self.volcanic_angles_deg = np.linspace(0.0, 360.0, num=self.volcanic_n_verts, endpoint=False)
                    self.volcanic_shape_jitter = 1.0 + np.random.uniform(-jitter, jitter, size=self.volcanic_n_verts)

                    # Start the weavolcanicther disturbance with a small radius
                    seed_radius_nm = float(np.random.uniform(0.5 * min_r_nm, min_r_nm))
            ...
            if hasattr(self, "volcanic_active"):
                # Remove any active volcanic disturbance if the flag is turned off
